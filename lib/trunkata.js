...
function trunkata(root, options) {
  ...
  if (height > maxHeight) {
    ...
    for (; i >= 1; i--) {
      ...
      node = depthTree[i];
      console.log('z', node, node.nodeType);
      if (node.nodeType === 3) {
      ...
      // Otherwise, remove the child from the parent node
        depthTree[i - 1].removeChild(node);
        // TODO: Check height and set success if applicable
      }
